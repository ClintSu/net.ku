...
namespace NET.Utilities.Recursion.Tree
{
    /// <summary>
    ///     树形结构递归扩展方法工具类
    /// </summary>
    public static class TreeRecursionExtension
    {
        ...
        /// <returns>上级节点集</returns>
        /// <remarks>包含上级的上级的上级...</remarks>
        public static IEnumerable<T> GetAllDeepParentNodes<T>(this T vertex) where T : ITree<T>
        {
        ...
        /// <returns>下级节点集</returns>
        /// <remarks>包含下级的下级的下级...</remarks>
        public static IEnumerable<T> GetAllDeepSubNodes<T>(this T vertex) where T : ITree<T>
        {
            IList<T> collection = new List<T>();
            RecurseSubNodes(vertex, collection);
        ...
        {
            var nodeArray = nodes == null ? new T[0] : nodes.ToArray();

            var collection = new HashSet<T>();

            foreach (var node in nodeArray)
            {
                foreach (var parentNode in node.GetDeepParentNodes())
                {
        ...
        private static void RecurseSubNodes<T>(T vertex, ICollection<T> collection) where T : ITree<T>
        {
            ...
            foreach (var subNode in vertex.SubNodes)
            {
