...
namespace NET.Utilities
{
    ...
    {
        #region 定义属性
        private byte[,] AesiSbox;
        private byte[,] AesSbox;
        private byte[,] AesRcon;
        ...
        /// 构造函数
        /// </summary>
        /// <param name="keySize">加解密密钥类型</param>
        /// <param name="keyBytes">密钥</param>
        public AesBase(KeySize keySize,byte[] keyBytes)
        {
            BuildMatrix();//建立矩阵
            SetNbNkNr(keySize); //初始化Nb,Nk,Nr
            this.key = new byte[this.Nk*4];  // 16, 24, 32 bytes
            keyBytes.CopyTo(key, 0); //字符串拷贝函数，把keyBytes字符复制到key中
            KeyExpansion(); //处理Key
        }
        /// <summary>
        /// 初始化矩阵
        ...
        private void KeyExpansion()
        {
            ...
            for (row = Nk; row < Nb*(Nr + 1); row++)
            {
                ...
                if (row%Nk == 0) //逢nk时，对当前列的前一列作特殊处理  
                {
                    ...
                    temp[0] = (byte) ((int) temp[0] ^ (int) AesRcon[row/Nk, 0]);
                    temp[1] = (byte) ((int) temp[1] ^ (int) AesRcon[row/Nk, 1]);
                    temp[2] = (byte) ((int) temp[2] ^ (int) AesRcon[row/Nk, 2]);
                    temp[3] = (byte) ((int) temp[3] ^ (int) AesRcon[row/Nk, 3]);
                }
        ...
        private byte[] SubWord(byte[] word)
        {
            ...
            result[0] = this.AesSbox[word[0] >> 4, word[0] & 0x0f];
            result[1] = this.AesSbox[word[1] >> 4, word[1] & 0x0f];
            result[2] = this.AesSbox[word[2] >> 4, word[2] & 0x0f];
            result[3] = this.AesSbox[word[3] >> 4, word[3] & 0x0f];
            return result;
        }
        /// <summary>
        /// 密钥移位函数
        ...
        public byte[] Cipher(byte[] input)
        {
            ...
            for (int j = 0; j < blockNum; ++j)
            {
                ...
                {
                    this.State[i % 4, i / 4] = input[inputIndex];
                    ++inputIndex;
                }
                //No1:开始
                AddRoundKey(0);
                ...
                for (int i = 0; i < (4 * Nb); ++i)
                {
                    output[outputIndex] = this.State[i % 4, i / 4];
                    ++outputIndex;
                }
            }
            return output;
        ...
        private void SubBytes()
        {
            for (int r = 0; r < 4; ++r)
            {
                for (int c = 0; c < 4; ++c)
                {
                    this.State[r, c] = this.AesSbox[(this.State[r, c] >> 4), (this.State[r, c] & 0x0f)];
                }
            }
        }
        /// <summary>
        ...
        private void InvSubBytes()
        {
            for (int r = 0; r < 4; ++r)
            {
                for (int c = 0; c < 4; ++c)
                {
                    this.State[r, c] = this.AesiSbox[(this.State[r, c] >> 4), (this.State[r, c] & 0x0f)];
                }
            }
        }
        /// <summary>
    ...
    public class AesSecurity
    {
        ...
        {
            _security = Encoding.Default.GetBytes(key);
            switch (key.Length)
            {
                case 16:
                    _keySize = KeySize.Bits128;
                    break;
                case 24:
                    _keySize = KeySize.Bits192;
                    break;
                case 32:
                    _keySize = KeySize.Bits256;
                    break;
                default:
                    break;
            }
        ...
        /// 加密的字节
        /// </summary>
        /// <param name="block">加密的字节</param>
        /// <returns>加密后的字节</returns>
        public byte[] Encrypt(byte[] block)
        {
            try
            {
                AesBase aes = new AesBase(_keySize, _security);
                return aes.Cipher(block);
            }
            catch (Exception)
            {
                return block; //未加密
