...
namespace NET.Security
{
    public class Des
    {
        ...
        public static string EncryptString(string encryptString, string key)
        {
            ...
            byte[] keyIV = bytes;
            return Convert.ToBase64String(Des.EncryptBytes(Encoding.UTF8.GetBytes(encryptString), bytes, keyIV));
        }

        public static string EncryptString(string encryptString)
        {
            return Des.EncryptString(encryptString, Des.key);
        }
        ...
        public static byte[] EncryptBytes(byte[] sourceBytes, byte[] keyBytes, byte[] keyIV)
        {
            ...
            keyBytes = Des.CheckByteArrayLength(keyBytes);
            keyIV = Des.CheckByteArrayLength(keyIV);
        ...
        public static string DecryptString(string decryptString, string key)
        {
            ...
            byte[] bytes = Encoding.UTF8.GetBytes(key);
            byte[] keyIV = bytes;
            return Encoding.UTF8.GetString(Des.DecryptBytes(Convert.FromBase64String(decryptString), bytes, keyIV));
        }

        public static string DecryptString(string decryptString)
        {
            return Des.DecryptString(decryptString, Des.key);
        }
